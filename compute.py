...
multiplier = []
temp = 1
for i in range(len(list(strategies))):
...
def find_strongly_dominant_eq(
    playerno, totalplayer, topplayer, strategyarr=[], eqindex=-1
):
    if len(totalplayer) >= 1:
        ...
        for strategy in range(strategies[cur_player]):
            ...
            temp = find_strongly_dominant_eq(
                playerno, totalplayer, topplayer, temparray, eqindex
            )
...
def find_weakly_dominant_eq(playerno, totalplayer, topplayer, eqindex, strategyarr=[]):
    if len(totalplayer) >= 1:
        ...
        for strategy in range(strategies[cur_player]):
            ...
            # print "Tempar ", temparray
            temparray.append(strategy)
            temp, eqindex = find_weakly_dominant_eq(
                playerno, totalplayer, topplayer, eqindex, temparray
            )
            # print "Temp value ", temp
            if temp == -sys.maxint:
                return temp, eqindex
...
if return_value == -1:
    print(
        "Strongly dominant strategy equilibrium (in order of P1, P2, ... ,Pn) is:",
    )
    for i in strong_eq:
        print(
            i,
    ...
    for i in range(num_players):
        ...
        tempplayerlist.remove(i)
        result_index = [-1]
        value, result_index = find_weakly_dominant_eq(
            i, tempplayerlist, tempplayerlist[0], result_index
        )
